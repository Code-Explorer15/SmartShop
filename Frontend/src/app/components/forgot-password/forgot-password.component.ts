...
export class ForgotPasswordComponent {
  ...
  constructor(
    ...
    this.forgotPasswordForm.get('mobileNumber')?.valueChanges.subscribe(value => {
      if (value) {
        const formatted = this.formatPhoneNumber(value);
        if (formatted !== value) {
          this.forgotPasswordForm.get('mobileNumber')?.setValue(formatted, { emitEvent: false });
        }
      }
    });
  }
  ...
  updatePhoneValidation() {
    const phoneControl = this.forgotPasswordForm.get('mobileNumber');
    if (phoneControl) {
  ...
  onCountryChange(country: Country) {
    ...
    this.forgotPasswordForm.patchValue({ countryCode: country.dialCode });
    this.showCountryDropdown = false;
    this.forgotPasswordForm.patchValue({ mobileNumber: '' });
    this.updatePhoneValidation();
  }
  ...
  onSubmit() {
    if (this.forgotPasswordForm.valid) {
      ...
      const phoneDigits = this.forgotPasswordForm.value.mobileNumber.replace(/\D/g, '');
